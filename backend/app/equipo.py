...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#FFFFFF;border-radius:12px;overflow:hidden;font-family:Arial,Helvetica,sans-serif;color:#242A36">
  ...
  <tr><td style="padding:28px">
    <p style="margin:0 0 14px;font-size:20px;font-weight:bold;color:#1E2533">{escape(saludo)}</p>
    {"".join(f'<p style="margin:0 0 14px;font-size:15px;line-height:1.6">{escape(p)}</p>' for p in parrafos)}
    <p style="margin:24px 0"><a href="{escape(enlace)}" style="display:inline-block;background:#F2A81C;color:#2C3444;font-weight:bold;font-size:15px;text-decoration:none;padding:13px 24px;border-radius:999px">{escape(boton)}</a></p>
    <p style="margin:0;font-size:13px;line-height:1.6;color:#6B7180">{escape(pie)}</p>
  </td></tr>
